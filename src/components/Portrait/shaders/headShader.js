...
const headShader = `
  ...
  void main() {
    ...
    vec3 specularColor = keyLightBrightness * vec3(10.0/255.0, 10.0/255.0, 10.0/255.0)  * pow(max(dot(reflect(-lightDirection, vNormal), normalize(vViewPosition)), 0.0), 5.0);
    vec3 specularColor2 = keyLightBrightness / 5.0 * vec3(59.0/255.0, 59.0/255.0, 59.0/255.0) * pow(max(dot(reflect(lightDirection, vNormal), normalize(vViewPosition)), 0.0), 5.0);
